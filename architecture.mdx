...
<Accordion icon="browser" title="Frontend">
  ...
  * **Autenticação** : AWS Cognito ou Firebase para gerenciamento de usuários e autenticação segura.
</Accordion>
...
<Accordion icon="database" title="Banco de Dados">
  ...
  * **NoSQL :** DynamoDB ou MongoDB para dados não estruturados (transações, logs, eventos, histórico mensagens).
...
<Accordion icon="database" title="Filas e Processamento Assíncrono">
  * **AWS SQS/SNS ou Apache Kafka :** Para processamento assíncrono de eventos (ex.: webhooks, notificações).
</Accordion>
